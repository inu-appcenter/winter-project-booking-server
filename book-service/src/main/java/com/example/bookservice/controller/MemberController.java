...
public class MemberController {
    ...
    public MemberResponse findById(@PathVariable Long memberId) {
        log.info("[findById] 회원 조회를 수행합니다. id : {}", memberId);
        Member member = memberService.findById(memberId);
        log.info("[findById] 회원 조회를 완료했습니다. id : {}", member.getId());
        return new MemberResponse(member);
    }

    // 회원 삭제
    @DeleteMapping("/{memberId}")
    public ResponseResult delete(@PathVariable Long memberId) {
        log.info("[delete] 회원 탈퇴를 수행합니다. id : {}" , memberId);
